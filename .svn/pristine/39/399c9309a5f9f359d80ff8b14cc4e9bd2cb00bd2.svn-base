...
public class PoPTimeManager {
	private PopupWindow pop;
	private View popView;
	private static volatile PoPTimeManager instance;
	private TextView tvOne, tvTwo;

	private PoPTimeManager() {
	}
	 ...
	 */
	public static PoPTimeManager getInstance() {
		if (instance == null) {
			instance = new PoPTimeManager();
		}
		return instance;
	}
	...
	public View getPopView(Context context) {
		if (popView == null) {
			...
			// 引入窗口配置文件
			popView = inflater.inflate(R.layout.pop_tabhost_time, null);
		}
		return popView;
	}
